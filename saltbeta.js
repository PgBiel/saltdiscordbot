...
var fs = require("fs");
process.chdir("./Documents/Bot Stuff/SaltBeta");
...
bot.on("message", message => {
    ...
    if (/^beta.prefix(\s(?:.+))?$/i.test(input)) {
        console.log("Potential");
        try {
        prefixtest = input.match(/^beta.prefix(\s(?:.+))?$/)[1]; } catch(err) {
            prefixtest = 1;
        }
    ...
    if (/^beta.eval\s([^]+)$/i.test(input)) {
        if (message.author.id == ownerID) {
            var functionToEval = input.match(/^beta.eval\s([^]+)$/i)[1];
