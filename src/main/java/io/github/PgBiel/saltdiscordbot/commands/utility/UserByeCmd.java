...
public class UserByeCmd extends UserCommand {
    public UserByeCmd() {
        super("Say Hi");
        ...
        return context
                ...
                .then(
                    target.getPrivateChannel()
                        .flatMap(ch -> ch.createMessage("The user " + author.getMention() + " is saying bye to you!")
                                    .withAllowedMentions(AllowedMentions.builder().allowUser(author.getId()).build()))
                        .flatMap(ignore -> context.getEvent().createFollowup("Sent bye to " + target.getMention() + " successfully.").withEphemeral(true))
                        .onErrorResume(err -> context.getEvent().createFollowup("Could not send bye for some reason...").withEphemeral(true).then(Mono.error(err)))
                        .thenReturn(context));
    }
