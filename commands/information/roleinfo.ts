...
export type RoleInfoDummy = AInfoDummy & {
  role?: Role;
};

const func: TcmdFunc<RoleInfoDummy> = async function(msg, {
  ...
  const { android, action, arg: _arg, trArg, role: dummyRole } = dummy || {} as never;
  ...
  const color: string = (role.hexColor ||
    Constants.colors.ROLE_DEFAULT) === Constants.colors.ROLE_DEFAULT ?
      ...
      role.hexColor;
  const isDefault: boolean = color === Constants.colors.ROLE_DISPLAY_DEFAULT;
