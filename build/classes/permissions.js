...
class Permz {
    ...
    checkPerm(member, guildId, perm, isDefault = false) {
        ...
        const perms = deps_1.db.table("perms").get(guildId, []).filter((item) => item.type === "member" && item.id === member.id);
        let hasPerm = false;
        let setPerm = false;
        ...
        if (!filtered) {
            ...
            for (const role of roles) {
                const roleperms = deps_1.db.table("perms").get(guildId, []).filter((it) => it.type === "role" && it.id === role.id);
    ...
    isDisabled(guildId, channelid, name) {
        const disabled = deps_1.db.table("perms").get(guildId, [])
            .find((item) => (item.command === "any" || item.command === name)
