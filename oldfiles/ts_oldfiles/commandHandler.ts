  ...
  if (msg.channel.type === "text" && !(msg.author.bot)) {
    (() => {
      ...
      const instruction = input.replace(
        new RegExp("^" + _.escapeRegExp(usingPrefix ? "+" : (mentionfix || prefix)), "i"), "");
      const olPrefix = prefix;
      const exeFunc = async (cmd?, usesPrefix: boolean = false) => {
      ...
      if (usingPrefix) {
        return exeFunc(bot.commands.prefix, true);
      }
      theGrandObject.instruction = instruction;
      logger.debug("inst:", instruction);
      for (const cmdn in bot.commands) {
        if (bot.commands.hasOwnProperty(cmdn)) {
          ...
          if (
            ...
            return exeFunc(cmd).catch(rejct);
          }
        }
      }
